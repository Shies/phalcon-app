...
class Email
{
    ...
    {
        $config = Di::getDefault()->get('config')->smtp;
        $this->set('email_server', $config->host);
        $this->set('email_port', $config->port);
        $this->set('email_user', $config->username);
        $this->set('email_password', $config->password);
        $this->set('email_from', $config->from);
        $this->set('site_name', "1234");
    ...
    {
        $message = preg_replace("/href\=\"(?!http\:\/\/)(.+?)\"/i", 'href="www.wujie.com\\1"', $message);
        $tmp = "";
        $tmp .= "<html><head>";
    ...
    private function header($from = '')
    {
        pr($this->email_from);
        if ($from == '') {
            $from = '=?UTF-8?B?' . base64_encode($this->site_name) . "?= <" . $this->email_from . ">";
        } else {
        ...
        $header .= "X-Mailer: WuJie {$this->email_delimiter}";
    ...
    {
        $isDebug = Di::getDefault()->get('config')->debug;
        if ($isDebug === true) {
